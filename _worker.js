...
async function handleUserBotInteraction(tgApi, chatId, callerId, msgId, msgText, cbData, linkedUser, env, ctx, t, langCode, hostName, update, tgState) {
    ...
    const send = async (text, kb, editMsgId) => {
        ...
        try {
            ...
            return await fetch(`${tgApi}/sendMessage`, { 
                method: 'POST', 
                headers: { 'Content-Type': 'application/json' }, 
                body: JSON.stringify(payload) 
            });
        } catch (e) {
            console.error('[TG Bot] send failed:', e);
        }
    };
    ...
    const getMainMenu = () => {
        ...
        const text = isFA 
            ? `👋 <b>سلام ${uu?.name || ''}!</b>
${SEP}
👤 <b>شناسه:</b> <code>${uu?.id.substring(0,8)}</code>
💳 <b>موجودی:</b> <code>${balance.toLocaleString()}</code> تومان
📦 <b>سرویس‌ها:</b> <code>${activeSvcs.length}</code> فعال
📊 <b>مصرف:</b> <code>${usedGB}</code> GB
${SEP}`
            : `👋 <b>Hello ${uu?.name || ''}!</b>
${SEP}
👤 <b>ID:</b> <code>${uu?.id.substring(0,8)}</code>
💳 <b>Wallet:</b> <code>${balance.toLocaleString()}</code> IRT
📦 <b>Services:</b> <code>${activeSvcs.length}</code> active
📊 <b>Usage:</b> <code>${usedGB}</code> GB
${SEP}`;
        const kb = { inline_keyboard: [
    ...
    if (cbData === 'u_main' || msgText === '/start') {
        const { text, kb } = getMainMenu();
        await send(text, kb, cbData === 'u_main' ? msgId : null);
        if (cbData) await answerCb();
        return new Response('OK', { status: 200 });
    }
    
    // Default response to avoid hanging
    return new Response('OK', { status: 200 });
}
